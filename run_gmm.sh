...
if [ $stage -le 0 ]; then
  ...
  local/make_train.pl $data_root ./data
  local/make_eval.pl $data_root ./data
fi
...
if [ $stage -le 2 ]; then
  ...
  sid/train_diag_ubm.sh --cmd "$train_cmd" \
    --nj $nj --num-threads 8 --subsample 1 --num-gselect 30 --initial-gauss-proportion 0.5 \
    data/train 512 \
    exp/diag_ubm

  sid/train_full_ubm.sh --cmd "$train_cmd" \
    --nj $nj --remove-low-count-gaussians false --subsample 1 --num-gselect 20 \
    data/train \
...
if [ $stage -le 7 ]; then
  ...
  local/plda_scoring.sh --use-lda false data/train data/eval_enroll data/eval_test \
